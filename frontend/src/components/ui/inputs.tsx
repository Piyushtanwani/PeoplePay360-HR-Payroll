...
export function PercentInput({ value, onChange, id }: { value: number | null; onChange: (v: number) => void; id?: string }) {
  return (
    <div className="flex h-9 items-center rounded-control border border-separator bg-surface px-3 focus-within:border-accent">
      <input
        id={id}
        type="number"
        step="0.01"
        ...
        className="tnum w-full bg-transparent text-body outline-none"
      />
      <span className="ml-2 text-sm2 text-label2">%</span>
    </div>
  )
}

export function TimeField({ value, onChange, disabled, id }: { value: string; onChange: (v: string) => void; disabled?: boolean; id?: string }) {
  return <input id={id} type="time" value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)} className={cn(inputBase, 'tnum')} />
}
...
/** Month picker built from Selects so no date string is ever typed by hand. */
export function MonthPicker({ value, onChange, months = 18, className }: { value: string; onChange: (period: string) => void; months?: number; className?: string }) {
